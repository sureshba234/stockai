...
import { Newspaper, BarChart, Bell, AlertCircle } from 'lucide-react';
import { getStockData, type StockDataOutput } from "@/ai/flows/get-stock-data";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setError(`Failed to fetch data for ${queryTicker}. Please try another symbol.`);
      } finally {
        setIsLoading(false);
      }
    }
                        ...
                        {stockData.news.map((item, index) => (
                          <div key={index} className="flex items-start gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">{item.source} - {new Date(item.publishedAt).toLocaleDateString()}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <CardContent className="pt-2">
                    {stockData.predictions ? (
                      <div className="flex items-start gap-4">
                         <BarChart className="w-6 h-6 text-muted-foreground mt-1" />
                        <p className="text-muted-foreground">{stockData.predictions}</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center justify-center text-center p-8 border-2 border-dashed rounded-lg">
